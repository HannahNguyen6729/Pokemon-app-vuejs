...
Vue.createApp({
  ...
  watch: {
    "gameData.selectedDeck": function (newValue, oldValue) {
      ...
      //console.log("watch", this.gameData.selectedDeck);
    },
  ...
  methods: {
    ...
    handleResetGame() {
      ...
      const newPokemonCardList = [];

      //double pokemons in the poke list
      pokemonCardList.map((pokemonCard) => {
        const pokemonCard1 = { ...pokemonCard };
        const pokemonCard2 = { ...pokemonCard };
        newPokemonCardList.push(pokemonCard1);
        newPokemonCardList.push(pokemonCard2);
      });
      // console.log(newPokemonCardList.length);

      this.pokeList = newPokemonCardList.sort(() => Math.random() - 0.5);
      this.selectedCards = [];
      this.pairedCards = [];
      this.gameResult = {
        win: false,
    ...
    createPokemonByDeck() {
      let arrayOfCards = [];
      const numberOfPoke = this.gameData.selectedDeck;
      for (let i = 0; i < numberOfPoke; i++) {
        const index = getRandomIntInclusive(0, 99);
        const randomPokeCard = pokemonList[index];
        arrayOfCards.push(randomPokeCard);
      }
